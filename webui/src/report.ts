export type ReportPayload = {
  room: { title: string; description?: string; publicSlug?: string };
  brief?: { markdown: string };
  sources: { title?: string; url: string }[];
  claims: { claimText: string; quote: string }[];
  contradictions: { disagreementType: string; explanation: string }[];
};

export async function loadReport(slug: string): Promise<ReportPayload | null> {
...
export function renderReport(payload: ReportPayload) {
  ...
  if (titleEl) titleEl.textContent = payload.room.title;
  if (subtitleEl) {
  ...
  if (sourcesEl) {
    sourcesEl.innerHTML = payload.sources
      .map(
        (source) =>
          `<li><a href="${source.url}" target="_blank">${source.title || source.url}</a></li>`
      )
      .join('');
  }
  ...
  if (claimsEl) {
    claimsEl.innerHTML = payload.claims
      .map(
        (claim) =>
          `<li><strong>${claim.claimText}</strong><br/><em>${claim.quote}</em></li>`
      )
      .join('');
  }
  ...
  if (contradictionsEl) {
    contradictionsEl.innerHTML = payload.contradictions
      .map(
        (item) =>
          `<li><strong>${item.disagreementType}</strong><br/>${item.explanation}</li>`
      )
      .join('');
  }
}
